...
$('#promo-add-form').submit( function(e) {
	...
	axios.post('/ajax/inventory/deals', {
...
function getPromoDetails(id)
{
	...
	let loading = new ModalLoading('#edit-deals-modal');
	loading.setLoading();

	axios.get(`/ajax/inventory/deals/${ dealId }`)
		.then((response) => {
			...
			$('#--product-details').html(`
				<div class="text-center">
					<img src="${ promoDetails['product']['product_img'] }" class="image-150 img-thumbnail"><br>
				
					<div class="name">
				        <h4 class="text-primary lighter">${ promoDetails['product']['product_name'] }</h4>
				        <h4 class="text-muted lighter">${ promoDetails['product']['brand_name'] }</h4>
				        <h4 class="text-primary lighter">${ promoDetails['product']['unit_price_format'] }</h4>
				    </div>
				</div>
			`);
...
$('#promo-edit-form').submit(function(e){
	...
	axios.patch(`/ajax/inventory/deals/${ dealId }`, {
